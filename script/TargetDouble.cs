...
public class TargetDouble : WindowBase {
	...
	{
		m_ctrlMessage.Initialize ("タップ効果アップ！", "動画を視聴して5分間タップの経験地２倍！");
		...
		case STEP.SHOW_AD:
			if (bInit) {
				if (false == UnityAdsSupporter.Instance.ShowRewardedAd ()) {
					...
					Debug.Log (end_time);
					DataManager.Instance.kvs_data.Write (DataManager.Instance.KEY_TARGET_DOUBLE_END, end_time);
